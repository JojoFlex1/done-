...
var OfflineFetcher = class _OfflineFetcher {
  ...
  async fetchHandle(handle) {
    try {
      const assetName = (0, import_common.fromUTF8)(handle.replace("$", ""));
      const handleAsset = `${import_common.SUPPORTED_HANDLES[1]}000de140${assetName}`;
  ...
  async fetchHandleAddress(handle) {
    const assetName = (0, import_common.fromUTF8)(handle.replace("$", ""));
    const policyId = import_common.SUPPORTED_HANDLES[1];
  ...
  addSerializedTransaction(txHex) {
    const tx = import_core_cst.Serialization.Transaction.fromCbor(import_core_cst.Serialization.TxCBOR(txHex));
    const time = Date.now();
    const slot = (0, import_common.resolveSlotNo)(this.network ?? "mainnet", time);
    const epoch = (0, import_common.resolveEpochNo)(this.network ?? "mainnet", time);
  ...
  slotToEpochSlot(slot) {
    const slotConfig = import_common2.SLOT_CONFIG_NETWORK[this.network ?? "mainnet"];
...
var parseAssetUnit = (unit) => {
  const policyId = unit.slice(0, import_common3.POLICY_ID_LENGTH);
  const assetName = unit.includes(".") ? (0, import_common3.fromUTF8)(unit.split(".")[1] || "") : unit.slice(import_common3.POLICY_ID_LENGTH);
    ...
    if (scriptHash) {
      ...
      if (status === 200 || status == 202) {
        ...
        if (data.type.startsWith("plutus")) {
          ...
          const normalized = (0, import_core_cst2.normalizePlutusScript)(plutusScript, "DoubleCBOR");
          script = {
        ...
        return (0, import_core_cst2.toScriptRef)(script).toCbor().toString();
      }
      throw parseHttpError(data);
    }
    return void 0;
  ...
  resolveScriptRef = (kScriptRef) => {
    if (kScriptRef) {
      ...
      if (kScriptRef.type.startsWith("plutus")) {
        const normalized = (0, import_core_cst3.normalizePlutusScript)(
          kScriptRef.bytes,
          "DoubleCBOR"
        );
        script = {
...
var U5CProvider = class {
   ...
   */
  async fetchAddressUTxOs(address, asset) {
    const addressBytes = (0, import_common8.hexToBytes)(import_core_cst5.Address.fromBech32(address).toBytes());
  ...
  async fetchUTxOs(hash, index) {
    ...
    if (index !== void 0) {
      const [utxo] = await this.queryClient.readUtxosByOutputRef([{ txHash, outputIndex: index }]);
  ...
  _rpcUtxoToMeshUtxo(rpcTxoRef, rpcTxOutput) {
    ...
    if (rpcTxOutput.script !== void 0) {
      if (rpcTxOutput.script.script.case !== "native" && rpcTxOutput.script.script.value) {
        ...
        scriptHash = (0, import_core_cst5.deserializePlutusScript)(scriptRef, V).hash();
      }
    }
    return {
      input: {
